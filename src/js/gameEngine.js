...
function gameLoop(state, game, timestamp) {
    ...
    if (state.keys.Space) {
        ...
        if (timestamp > state.fireball.nextSpawnTimestamp) {//проверяваме дали
            //текущия timestamp е > от nextTimeStamp и тогава създаваме нов fireball;
            game.createFireball(wizard, state.fireball);
            state.fireball.nextSpawnTimestamp = timestamp + state.fireball.fireRate;
            //правим времеви интервал м/у изстрелването на fireballs;
        }
